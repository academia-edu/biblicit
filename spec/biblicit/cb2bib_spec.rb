...
describe Cb2Bib do
  ...
  unless ENV['LOCAL']
    ...
    it "parses 'Multi-scale collaborative...' headers from file" do
      result = Biblicit.extract(file: "#{FIXTURES_DIR}/pdf/ICINCO_2010.pdf", tools: [:cb2bib], remote: true)

      parsed = result[:cb2bib]
  ...
  it "parses 'Multi-scale collaborative...' headers from content" do
    ...
    result = Biblicit.extract(content: content, tools: [:cb2bib], remote: false)
    parsed = result[:cb2bib]
  ...
  unless ENV['LOCAL']
    ...
    it "parses 'Oligopoly, Disclosure...' headers" do
      result = Biblicit.extract(file: "#{FIXTURES_DIR}/pdf/Bagnoli Watts TAR 2010.pdf", tools: [:cb2bib])
      parsed = result[:cb2bib]
    ...
    it "parses Google paper headers" do
      result = Biblicit.extract(file: "#{FIXTURES_DIR}/pdf/10.1.1.109.4049.pdf", tools: [:cb2bib])
      parsed = result[:cb2bib]
    ...
    it "handles docx file" do
      ...
      result = Biblicit.extract(file: "#{FIXTURES_DIR}/Review_of_Michael_Tyes_Consciousness_Revisited.docx", tools: [:cb2bib])
      header = result[:cb2bib]
      header[:valid].should be_true
    end

    it "handles ps file" do
      result = Biblicit.extract(file: "#{FIXTURES_DIR}/KerSch99.ps", tools: [:cb2bib])
